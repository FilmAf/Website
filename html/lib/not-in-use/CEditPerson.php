...
class CEditPerson extends CEditObj
{
...
//		$this->mb_allow_redirect	= false;
		...
		$this->ms_title				= $this->mn_edit_mode == CWnd_EDIT_NEW ? 'Submit New Person' : 'Submit Person Change';
		$this->mn_template			= DVDAF_EDIT_PERSON;
		$this->mn_select_mode		= DVDAF_SELECT_PERSON;
	...
	function getFooterJavaScript()
	{
		...
		$s_config  = '{baseDomain:"'.					$this->ms_base_subdomain.'"'.
					 ...
					 ',onPopup:PersonEdit.onPopup'.
					 ',ulExplain:1,ulBirth:1,ulPerson:1'.
					 ',imgPreLoad:"explain.undo.spin"'.
					 '}';
		return
					"function setSearchVal(a,b,c,d){PersonEdit.setSearchVal(a,b,c,d)};".
					"function validate(a){PersonEdit.validate(a)};".
					"function onMenuClick(action){PersonEdit.onClick(action)};".
