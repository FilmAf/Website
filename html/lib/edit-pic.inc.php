...
function drawPicSubCurr(&$ln, $s_def_pic, $b_options)
{
	if ( $ln['pic_name'] && $ln['pic_name'] != '-' )
	{
		...
		$s_curr_pic_name = "<img src='".CPic::location($s_curr_pic_name,CPic_THUMB)."?tm={$s_rand}'>".
						   ($b_options ? "<br /><input id='picp_{$s_curr_id}' type='button' value='Options' class='mp'>" : '');
	}
	else
	{
		$s_curr_pic_name = "<img src='http://dv1.us/d1/1.gif' width='63' height='90' />";
	}
...
function drawPicSubProp(&$ln, $s_pic_dispo_cd, $s_request_cd, $s_base_subdomain, &$s_prop_id, $b_options, $b_is_proposer)
{
	if ( $ln['pic_edit_id'] )
	{
		...
		if ( $ln['disposition_cd'] == 'A' && $ln['pic_name'] != '-' )
			$s_src = CPic::location($ln['pic_name'],CPic_THUMB);
		else
	...
	{
		$s_prop_pic_name = "<img src='http://dv1.us/d1/1.gif' width='63' height='90' />";
		$s_prop_id		 = '';
	}
	...
	if ( $ln['pic_id'] && $b_curr )
	{
		...
		$s_orig = "<div class='qa'>".
				...
				"<div style='margin-left:20px'>".
				   ...
				   ( $s_orig ? "<table class='no_border'>{$s_orig}</table>" : '' ).
				"</div>".
			  "</div>";
	}
...
//	}
	...
	$s_prop = ( $ln['disposition_cd'] == '-'
				...
				"<div style='margin-left:20px'>".
				  ...
				  ( $s_prop ? "<table class='no_border'>{$s_prop}</table>" : '' ).
				"</div>".
			  "</div>";
	}
	...
	return "<td class='sg' style='vertical-align:top;padding-top:4px'>".
			 ($s_notes ? "<img src='http://dv1.us/d1/1.gif' height='1' width='380' />". $s_notes : '&nbsp;').
		   "</td>";
}
