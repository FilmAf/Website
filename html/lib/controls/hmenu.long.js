 ...
 *  Copyright Dynarch.com, 2003-2020.  All Rights Reserved.
 *  http://www.dynarch.com/
 *
 *  Graciously provided to Film Aficionado for use with this website
 *
...
(
	function()
	{
		...
		/*
		Ed Hoo changes begin
			_dynarch_top._dynarch_menu_url="/hmenu/";
		*/
			_dynarch_top._dynarch_menu_url="/lib/controls/";
		/*
		Ed Hoo changes end
		*/
		else
...
DynarchMenu.prototype.addIcon=function(info,item,icon)
{
	...
	var CE=DynarchMenu._createElement,
		...
		r=CE("tr",CE("tbody",t=CE("table",null,doc))),
		td1=CE("td",r),
		td2=CE("td",r);
...
DynarchMenu.prototype.createMenuItem=function(li,parent,horiz,arrow)
{
	var tmp,
		...
		self=this,
		key=null,
		disabled=/(^|\s+)disabled(\s+|$)/i.test(licl),
		nohover=DynarchMenu.getCA(li,"nohover"),
		win=parent.base?cfg.frames.main:cfg.frames.popups,
		doc=win.document,
		CE=DynarchMenu._createElement;
	if(cfg.d_profile)
...
DynarchMenu._documentMouseUp=function(ev)
{
	...
	if(DynarchMenu._msupTimeout)
		return false;
	var menu=_dynarch_top.DynarchMenu._C,
		el,
		info;
	if(menu)
	{
...
DynarchMenu._documentKeyPress=function(ev)
{
	...
	DynarchMenu._forAllMenus(function(menu)
	{
		...
		*/
		if ( ev )
		{
			switch ( (ev.srcElement && ev.srcElement.tagName) ? ev.srcElement.tagName : ((ev.target && ev.target.tagName) ? ev.target.tagName : '') )
			{
			case 'TEXTAREA':
			case 'INPUT':	return;
			}
...
DynarchMenu.prototype.createMenuTree=function(ul,horiz)
{
	var base=!this._baseMenuInfo,
		...
		submenu,
		CE=DynarchMenu._createElement,
		win=base?cfg.frames.main:cfg.frames.popups,
		doc=win.document;
	if(cfg.d_profile)
...
{
	var a=this._baseMenuInfo.element,
		i,
		el;
	try
	{
...
DynarchMenu._getPos=function(el)
{
	...
	*/
	...
	//	With:
	if(el.parentNode&&el.parentNode!==el.offsetParent&&!is_opera)
	/*
	Ed Hoo changes End
	*/
	{
...
*/
	...
	// With:
	for(;related;related=(related.parentNode ? related.parentNode : null) )
/*
Ed Hoo changes end
*/
		if(related===element)
		...
		*/
		{
			...
			setTimeout(function(){_dynarch_top.DynarchMenu._C=info.menu;},info.menu.config.timeout);
			DynarchMenu._stopEvent(ev);
			return false;
		}
	}
...
DynarchMenu.MenuTree.prototype.openContext=function(ev,trigger)
{
	...
	default:
		...
		Ed Hoo changes begin
		*/
		var e = el.getAttribute('dynarch_below');
		if ( e && (e = document.getElementById(e)) )
		{
			p=DynarchMenu._getPos(e);
			p.y+=e.offsetHeight;
			break;
		}
		/*
...
{
	/*
	Ed Hoo changes begin (adding this.doNotOpen that may be defined in the onPopup call)
	*/
	...
	*/
	...
	var m=this.element,
		self=this.menu,
		cfg=self.config,
		win=cfg.frames.popups,
		p=el?win.DynarchMenu._getPos(el):pos,
		pe,
	...
	*/
    if ( this.doNotOpen )
		return;
	if ( el && is_opera && typeof(el.cells) == "object" )
	...
	if(!(base&&cfg.crossFrames))
	{
		...
		else
		{
			if(!is_khtml)
			{
				...
				{
					/*
					Ed Hoo changes begin (in Safari el.lastChild may not be defined, adding a check to bypass it in that condition)
					*/
					if ( el.lastChild )
					{
...
/*
Ed Hoo changes begin
- Make sure the menu does not open left of the browser view window or below it
*/
	{
		var x0 = win.DynarchMenu.psLeft();
		if ( p.x < x0 ) p.x = x0;
	}
	{
		var yi = vw.y - m.offsetHeight, y0;
		if ( p.y > yi )
		{
			y0 = win.DynarchMenu.psTop();
			p.y = yi >= y0 ? yi : y0;
		}
	}
/*
...
DynarchMenu._nfo={product:"hmenu-2.9",licensee:"",license_key:"",purchase_date:"",license_type:""};
