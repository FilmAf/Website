...
function getDvdRights(&$obj)
{
    ...
    $rr = CSql::query_and_fetch("SELECT contributor_cd, membership_cd, moderator_cd FROM dvdaf_user WHERE user_id = '{$obj->ms_user_id}'", 0,__FILE__,__LINE__);
    if ( $rr )
    {
	$b_mod = intval($rr['moderator_cd']) >= 5;
	$rr['membership_cd'] = 0;
	$rr['contributor_cd'] = 0;
	...
	if ( ! $b_mod )
	{
	    ...
	    */
	    switch ( intval($rr['membership_cd']) )
	    {
		...
		if ( ($rr = CSql::query("SELECT edit_title, new_title, new_picture, count(*) count ".
					 ...
					 "WHERE user_id = '{$obj->ms_user_id}' and rejected != 'N' and update_tm > date_add(now(), INTERVAL -7 DAY) ".
		...
		if ( ($rr = CSql::query("SELECT edit_title, new_title, new_picture, count(*) count ".
					  "FROM dvd_direct_update ".
					 "WHERE user_id = '{$obj->ms_user_id}' and rejected = 'N' and update_tm > date_add(now(), INTERVAL -1 DAY) ".
					 "GROUP BY edit_title, new_title, new_picture", 0,__FILE__,__LINE__)) )
		{
		    while ( ($ln = CSql::fetch($rr)) )
		    {
