...
class CPicEdit extends CWnd
{
	...
	//	$this->mb_allow_redirect	= false;
		...
		$this->ms_include_css	   .=
			"<style type='text/css'>".
				"td.section "				."{ text-align:center; font-size:12px; padding:4px; font-weight:bold; color:#072b4b; background-color:#edf4fa; }".
			"</style>";
		...
		$this->ms_obj_type		= dvdaf3_getvalue('obj_type', DVDAF3_POST|DVDAF3_LOWER);
		$this->mc_obj_type		= '';
		...
		if ( $this->ms_obj_type )
		{
			$this->mb_dvd_submission	= dvdaf3_getvalue('pdvd'			  ,DVDAF3_POST|DVDAF3_BOOLEAN); // pdvd=1 if oppened from dvd submission processing, as opposed to picture management
			$this->ms_act				= dvdaf3_getvalue('act'			  ,DVDAF3_POST|DVDAF3_LOWER  ); // new, rep, edit, asnew
			$this->ms_step				= dvdaf3_getvalue('step'			  ,DVDAF3_POST|DVDAF3_LOWER  ); // upload, preview, save
			$this->mn_pic_id			= dvdaf3_getvalue('pic'			  ,DVDAF3_POST|DVDAF3_INT    );
			$this->mn_pic_edit_id		= dvdaf3_getvalue('n_p_pic_edit_id',DVDAF3_POST|DVDAF3_INT	   ); if ( ! $this->mn_pic_edit_id ) $this->mn_pic_edit_id = dvdaf3_getvalue('pic_edit', DVDAF3_POST|DVDAF3_INT);
			$this->ms_obj_type			= dvdaf3_getvalue('obj_type'		  ,DVDAF3_POST|DVDAF3_LOWER  );
			$this->mn_obj_id			= dvdaf3_getvalue('obj'			  ,DVDAF3_POST|DVDAF3_INT    );
			$this->mn_obj_edit_id		= dvdaf3_getvalue('obj_edit'		  ,DVDAF3_POST|DVDAF3_INT    );
			$this->ms_seed				= dvdaf3_getvalue('seed'			  ,DVDAF3_POST			   ); // 2009-10-10 21:08:50 192.168.1.100
			$this->ms_replace			= dvdaf3_getvalue('replace_pic'	  ,DVDAF3_POST|DVDAF3_LOWER  );
		}
		else
		{
			$this->mb_dvd_submission	= dvdaf3_getvalue('pdvd'			  ,DVDAF3_GET|DVDAF3_BOOLEAN );
			$this->ms_act				= dvdaf3_getvalue('act'            ,DVDAF3_GET|DVDAF3_LOWER   ); if ( ! $this->ms_act ) $this->ms_act = 'open'; // in preparation for edit (del, def, wdr, open)
			$this->ms_step				= '';
			$this->mn_pic_id			= dvdaf3_getvalue('pic'			  ,DVDAF3_GET|DVDAF3_INT     );
			$this->mn_pic_edit_id		= dvdaf3_getvalue('pic_edit'		  ,DVDAF3_GET|DVDAF3_INT     );
			$this->ms_obj_type			= dvdaf3_getvalue('obj_type'		  ,DVDAF3_GET|DVDAF3_LOWER   );
			...
			$this->ms_seed				= date('Y-m-d H:i:s '). dvdaf3_getvalue('REMOTE_ADDR', DVDAF3_SERVER);
			$this->ms_replace			= '';
		}
		$this->ms_pic_name		= '';
		...
		$this->ms_return_url	= $this->mb_dvd_submission ? "/utils/x-dvd-appr.html" : $this->ms_pic_mngt_url;
	}
	...
	function getFooterJavaScript()
	{
		...
		return		"function onMenuClick(action){PicEdit.onClick(action);};".
	...
	function validateReject()
	{
		...
		$s_reviewer_notes = dvdaf3_textarea2db(dvdaf3_getvalue('modjust', DVDAF3_POST), 1000);
	...
	function validateOpen()
	{
		...
		case 'E':
			...
			$s_dir				= CPic::dir($s_pic_name);
			$this->ms_base_pic	= "/p6/{$s_dir}/{$s_pic_name}.jpg";
			$this->ms_bord_pic	= "/p4/{$s_dir}/{$s_pic_name}.jpg";
			$this->ms_prev_pic	= $this->mb_review_sub
								  ? "/uploads/". sprintf('%06d',$this->mn_pic_edit_id) ."-prev.jpg"
								  : ($this->ma_data['uploaded_pic'] == '-' ? CPic::location($s_pic_name,CPic_PIC) : "/uploads/{$s_pic_name}-prev.jpg");
			$this->ms_act		= 'edit';
			break;
		case 'D':
			$s_pic_name			= $this->ma_data['pic_name'];
			...
			$this->ms_base_pic	= "/p6/{$s_dir}/{$s_pic_name}.jpg";
			$this->ms_bord_pic	= "/p4/{$s_dir}/{$s_pic_name}.jpg";
			$this->ms_prev_pic	= CPic::location($s_pic_name,CPic_PIC);
			$this->ms_act		= 'des';
			break;
		}
	}
	...
	function validateSave()
	{
		...
		$this->ma_data[$this->ms_col_DVD_ID]= $this->mn_obj_id;
		$this->ma_data['img_treatment']		= dvdaf3_getvalue('n_p_img_treatment' ,DVDAF3_POST          ,1); unset($_POST['n_p_img_treatment' ]);
		$this->ma_data['rot_degrees']		= dvdaf3_getvalue('n_p_rot_degrees'   ,DVDAF3_POST|DVDAF3_FLOAT); unset($_POST['n_p_rot_degrees'   ]);
		$this->ma_data['rot_degrees_x']		= dvdaf3_getvalue('n_p_rot_degrees_x' ,DVDAF3_POST|DVDAF3_INT  ); unset($_POST['n_p_rot_degrees_x' ]);
		$this->ma_data['crop_fuzz']			= dvdaf3_getvalue('n_p_crop_fuzz'     ,DVDAF3_POST|DVDAF3_INT  ); unset($_POST['n_p_crop_fuzz'     ]);
		$this->ma_data['crop_x1']			= dvdaf3_getvalue('n_p_crop_x1'       ,DVDAF3_POST|DVDAF3_INT  ); unset($_POST['n_p_crop_x1'       ]);
		$this->ma_data['crop_x2']			= dvdaf3_getvalue('n_p_crop_x2'       ,DVDAF3_POST|DVDAF3_INT  ); unset($_POST['n_p_crop_x2'       ]);
		$this->ma_data['crop_y1']			= dvdaf3_getvalue('n_p_crop_y1'       ,DVDAF3_POST|DVDAF3_INT  ); unset($_POST['n_p_crop_y1'       ]);
		$this->ma_data['crop_y2']			= dvdaf3_getvalue('n_p_crop_y2'       ,DVDAF3_POST|DVDAF3_INT  ); unset($_POST['n_p_crop_y2'       ]);
		$this->ma_data['black_pt']			= dvdaf3_getvalue('n_p_black_pt'      ,DVDAF3_POST|DVDAF3_INT  ); unset($_POST['n_p_black_pt'      ]);
		$this->ma_data['white_pt']			= dvdaf3_getvalue('n_p_white_pt'      ,DVDAF3_POST|DVDAF3_INT  ); unset($_POST['n_p_white_pt'      ]);
		$this->ma_data['gamma']				= dvdaf3_getvalue('n_p_gamma'         ,DVDAF3_POST|DVDAF3_FLOAT); unset($_POST['n_p_gamma'         ]);
		$this->ma_data['proposer_notes']	= dvdaf3_textarea2db(dvdaf3_getvalue('n_p_proposer_notes',DVDAF3_POST), 1000); unset($_POST['n_p_proposer_notes']);
		$this->ma_data['reviewer_notes']	= dvdaf3_textarea2db(dvdaf3_getvalue('n_p_reviewer_notes',DVDAF3_POST), 1000); unset($_POST['n_p_reviewer_notes']);
		...
		if ( ! $this->mn_pic_edit_id || $this->mb_review_sub )
		{
			...
			CValidate::validateInput($s_change, $s_values, DVDAF_INSERT, false, $this->ms_display_error);
			if ( $a_pic_submit )
			{
		...
		{
			// update old values from the database as what is in DVDAF3_POST may not agree with it because of the preview functionality
			...
			if ( $this->mn_obj_edit_id )
			{
				if ( ($s_proposer_notes = CSql::query_and_fetch1("SELECT proposer_notes FROM {$this->ms_tbl_DVD_SUB} WHERE edit_id = {$this->mn_obj_edit_id} and disposition_cd <> '-'",0,__FILE__,__LINE__)) )
				{
					if ( $s_proposer_notes == '-' )
						...
						$s_proposer_notes = dvdaf3_translatestring("AUTO GEN MSG - new picture uploaded<br />{$s_proposer_notes}", DVDAF3_NO_TRANSLATION, 1000);
				}
	...
	function validateSaveMod($s_seed)
	{
		...
		// propagate picture (no loger propagate, we only have one server)
		//$s_temp = '/var/www/html/uploads/filmaf.'.CTime::get_time().'.tmp';
		//copy("http://".CPic::server($s_pic_name)."dv1.us/utils/propagate.php?pic={$s_pic_name}", $s_temp);
		...
		$s_status = dvdaf3_getvalue('n_p_pic_type', DVDAF3_POST, 1) == 'P' ? 'P' : 'Y';
		if ( $s_request_cd == 'N' )
		{
	...
	function validateSavePrev($s_act, $n_pic_id, $n_pic_edit_id, $b_save)
	{
		...
		if ( ($this->ma_data = CSql::query_and_fetch($rr,0,__FILE__,__LINE__)) )
		{
			...
			else
			{
				...
				$this->ma_data['pic_type']		 = dvdaf3_getvalue('n_p_pic_type'      ,DVDAF3_POST          ,1);
				$this->ma_data['caption']		 = dvdaf3_getvalue('n_p_caption'       ,DVDAF3_POST            );
				$this->ma_data['copy_holder']	 = dvdaf3_getvalue('n_p_copy_holder'   ,DVDAF3_POST            );
				$this->ma_data['copy_year']		 = dvdaf3_getvalue('n_p_copy_year'     ,DVDAF3_POST|DVDAF3_INT  );
				$this->ma_data['suitability_cd'] = dvdaf3_getvalue('n_p_suitability_cd',DVDAF3_POST          ,1);
				$this->ma_data['img_treatment']	 = dvdaf3_getvalue('n_p_img_treatment' ,DVDAF3_POST          ,1);
				$this->ma_data['rot_degrees']	 = dvdaf3_getvalue('n_p_rot_degrees'   ,DVDAF3_POST|DVDAF3_FLOAT);
				$this->ma_data['rot_degrees_x']	 = dvdaf3_getvalue('n_p_rot_degrees_x' ,DVDAF3_POST|DVDAF3_INT  );
				$this->ma_data['crop_fuzz']		 = dvdaf3_getvalue('n_p_crop_fuzz'     ,DVDAF3_POST|DVDAF3_INT  );
				$this->ma_data['crop_x1']		 = dvdaf3_getvalue('n_p_crop_x1'       ,DVDAF3_POST|DVDAF3_INT  );
				$this->ma_data['crop_x2']		 = dvdaf3_getvalue('n_p_crop_x2'       ,DVDAF3_POST|DVDAF3_INT  );
				$this->ma_data['crop_y1']		 = dvdaf3_getvalue('n_p_crop_y1'       ,DVDAF3_POST|DVDAF3_INT  );
				$this->ma_data['crop_y2']		 = dvdaf3_getvalue('n_p_crop_y2'       ,DVDAF3_POST|DVDAF3_INT  );
				$this->ma_data['black_pt']		 = dvdaf3_getvalue('n_p_black_pt'      ,DVDAF3_POST|DVDAF3_INT  );
				$this->ma_data['white_pt']		 = dvdaf3_getvalue('n_p_white_pt'      ,DVDAF3_POST|DVDAF3_INT  );
				$this->ma_data['gamma']			 = dvdaf3_getvalue('n_p_gamma'         ,DVDAF3_POST|DVDAF3_FLOAT);
				$this->ma_data['proposer_notes'] = dvdaf3_textarea2db(dvdaf3_getvalue('n_p_proposer_notes',DVDAF3_POST), 1000);
				$this->ma_data['reviewer_notes'] = dvdaf3_textarea2db(dvdaf3_getvalue('n_p_reviewer_notes',DVDAF3_POST), 1000);
			}
			$this->ma_data[$this->ms_col_DVD_ID] = $this->mn_obj_id;
	...
	function delPreviousSeed()
	{
		...
		if ( ($rr = CSql::query("SELECT pic_edit_id, uploaded_pic FROM pic_submit WHERE proposer_id = '{$this->ms_user_id}' and creation_seed = '{$this->ms_seed}' and disposition_cd = '-'",0,__FILE__,__LINE__)) )
		{
			...
			for ( $i = 0 ; $i < count($an_pic_edit_id) ; $i++ )
			{
				...
				if ( $as_uploaded_pic[$i] ) dvdaf3_exec("rm -f /var/www/html/uploads/{$as_uploaded_pic[$i]}*.jpg /var/www/html/uploads/{$as_uploaded_pic[$i]}*.gif", $this->ma_cmdout, $this->mn_cmdret);
			}
		}
	}
		...
		echo  "<form id='myform' name='myform' method='post' action='{$this->ms_request_uri}'>".
			  "<table>".
				"<tr>".
				  "<td>".
					"<table class='border'>".
					  "<tr><td class='section'>Request Status</td></tr>".
					  "<tr>".
						"<td>";
						  dvdaf_getbrowserow($this->ma_data, $s_tmpl_top, 0, 0, $this->ms_user_id, DVDAF2_ECHO, DVDAF3_INPUT, DVDAF4_ZERO_KEY_OK);
		echo			"</td>".
					  "</tr>".
					"</table>".
					"<table class='border' style='margin:14px 0px 4px 0px'>".
					  "<tr><td class='section'>Picture Information</td></tr>".
					  "<tr>".
						"<td>".
						  "<table class='no_border'>".
							"<tr class='dvd_row0'>";
							  dvdaf_getbrowserow($this->ma_data, $s_tmpl_mid, 0, 0, $this->ms_user_id, DVDAF2_ECHO, 0, DVDAF4_ZERO_KEY_OK);
		echo		 	     "</td>".
							"</tr>".
						  "</table>".
						"</td>".
				...
				"<tr>".
				  "<td>".
					"<table class='border' cellpadding='4'>".
					  "<tr><td colspan='3' class='section'>Picture Processing</td></tr>".
					  ...
					  "<tr>".
						"<td style='vertical-align:top'>".
						  "<img style='padding:5px' src='{$this->ms_bord_pic}?tm={$s_rand}' />".
						"</td>".
						...
						"</td>".
						"<td style='vertical-align:top'>".
						  "<img style='padding:20px' src='{$this->ms_prev_pic}?tm={$s_rand}' />".
						"</td>".
					  "</tr>".
					"</table>".
				  "</td>".
	...
	function drawPicSaved()
	{
		...
		if ( $this->mb_mod )
		{
			...
			echo  "<div style='margin:20px;text-align:center'>".
				"<img style='padding:20px' src='".CPic::location($this->ms_prev_pic,CPic_PIC  )."?tm={$s_rand}' /> ".
				"<img style='padding:20px' src='".CPic::location($this->ms_prev_pic,CPic_THUMB)."?tm={$s_rand}' />".
			  "</div>";
		}
		else
		{
	...
	function drawPicEdit()
	{
		...
		$s_proposer_notes	= dvdaf3_db2textarea($this->ma_data['proposer_notes']);
		$s_reviewer_notes	= dvdaf3_db2textarea($this->ma_data['reviewer_notes']);
					...
					"<table class='border' style='margin:14px 0px 4px 0px'>".
					  ...
					  "<tr>".
						"<td>".
						  "<table class='no_border'>".
							"<tr class='dvd_row0'>";
							  dvdaf_getbrowserow($this->ma_data, $s_tmpl_mid, 0, 0, $this->ms_user_id, DVDAF2_ECHO, DVDAF3_INPUT, DVDAF4_ZERO_KEY_OK);
				...
				"<tr>".
				  "<td>".
					"<table class='border' cellpadding='4'>".
					  "<tr><td colspan='3' class='section'>Picture Processing</td></tr>".
					  "<tr>".
						"<td>".($s_transf_old  ? "<div class='one_lbl'>Previous transforms:</div><div style='margin-left:20px'>{$s_transf_old }</div>" : '&nbsp;' )."</td>".
						"<td>".($s_transf_prop ? "<div class='one_lbl'>Proposed transforms:</div><div style='margin-left:20px'>{$s_transf_prop}</div>" : '&nbsp;' )."</td>".
						"<td>".($s_transf_now  ? "<div class='one_lbl'>Tranforms as shown:</div><div style='margin-left:20px'>{$s_transf_now }</div>" : '&nbsp;' )."</td>".
					  "</tr>".
						  ...
						  "<table class='no_border'>".
							"<tr>".
							  "<td style='vertical-align:top;white-space:nowrap'>".
								...
								"<img id='ex_p_proposer_notes' src='http://dv1.us/d1/00/bq00.png' width='17' height='17' align='absbottom' alt='Explain' />".
							  "</td>".
							  "<td>".
								"<textarea id='n_p_proposer_notes' name='n_p_proposer_notes' cols='40' rows='2' {$s_prop_read}maxlength='1000' wrap='soft'>{$s_proposer_notes}</textarea>".
								...
								"<img id='zi_p_proposer_notes' src='http://dv1.us/d1/1.gif' align='top' />".
							  "</td>".
							"</tr>".
							"<tr>".
							  "<td style='vertical-align:top;white-space:nowrap'>".
								"<div class='one_lbl'>Reviewer notes:</div>".
								"<img id='ex_p_reviewer_notes' src='http://dv1.us/d1/00/bq00.png' width='17' height='17' align='absbottom' alt='Explain' />".
								$s_helpers.
							  "</td>".
							  "<td>".
								"<textarea id='n_p_reviewer_notes' name='n_p_reviewer_notes' cols='40' rows='2' {$s_revi_read}maxlength='1000' wrap='soft'>{$s_reviewer_notes}</textarea>".
								...
								"<img id='zi_p_reviewer_notes' src='http://dv1.us/d1/1.gif' align='top' />".
							  "</td>".
							"</tr>".
						  "</table>".
						"</td>".
		...
		echo			      "<td width='90%' style='white-space:nowrap'>".
								...
								"<img id='zi_p_img_treatment' src='http://dv1.us/d1/1.gif' align='top' />".
							  "</td>".
							  ...
							  "<td width='1%' style='padding:10px 10px 0 10px;text-align:center;vertical-align:top'>".
								"<img src='{$gif}?tm={$s_rand}' />".
							  "</td>".
							"</tr>".
						  "</table>".
						"</td>".
					  "</tr>";
					  dvdaf_getbrowserow($this->ma_data, $s_tmpl_sup, 0, 0, $this->ms_user_id, DVDAF2_ECHO, DVDAF3_INPUT, DVDAF4_ZERO_KEY_OK);
		echo	    "</table>".
				  "</td>".
				"</tr>".
			  "</table>".
