...
function snapHistory($n_dvd_id, $n_version_id)
{
...
function countDifferentGenres($n_genre, $n_imdb)
{
    if ( $n_genre > 0 && $n_imdb > 0 )
    {
	$s_imdb = sprintf('%07d', intval($n_imdb));
...
function getAffectedMessage($n_dvd_id, $s_prop_ids, $n_genre, $s_imdb, $s_base)
{
    ...
    $s_affected = "Changes saved as dvd_id <a href='/search.html?has={$n_dvd_id}&init_form=str0_has_{$n_dvd_id}' target='filmaf'>{$n_dvd_id}</a>.";
